...
def village_play(screen, sound, cursor, charac, state, back_button, quit_button):
    ...
    while running:
        ...
        if pygame.sprite.spritecollide(block1, group, False):
            block1.bump()
        if pygame.sprite.spritecollide(block2, group, False):
            block2.bump()
        if pygame.sprite.spritecollide(block5, group, False):
            block5.bump()
        if pygame.sprite.spritecollide(block6, group, False):
            block6.bump()
        if pygame.sprite.spritecollide(block7, group, False):
            block7.bump()
        if pygame.sprite.spritecollide(block8, group, False):
            block8.bump()
        if pygame.sprite.spritecollide(block9, group, False):
            block9.bump()
        if pygame.sprite.spritecollide(block12, group, False):
            block12.bump()
